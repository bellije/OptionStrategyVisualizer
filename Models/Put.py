...
class Put(Option):
    ...
    def compute_current_price(self, curr_time_to_maturity, curr_price):
        return self.strike * exp(-self.risk_free * curr_time_to_maturity) * norm.cdf(- self.d2(curr_time_to_maturity, curr_price)) - curr_price * norm.cdf(-self.d1(curr_time_to_maturity, curr_price))
    ...
    def compute_payoffs(self, prices):
        return np.maximum(self.strike - prices, 0) - self.compute_initial_price()
